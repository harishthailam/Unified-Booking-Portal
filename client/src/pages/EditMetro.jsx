...
const EditTrain = () => {
  ...
  const [metroName, setTrainName] = useState('');
  ...
  const fetchTrainData = async () =>{
    await axios.get(`http://localhost:6001/fetch-metro/${id}`).then(
      (response) =>{
        console.log(response.data);
        setTrainName(response.data.metroName);
        setTrainNum(response.data.metroNumber);
  ...
  const handleSubmit = async () =>{
    ...
    const inputs = {metroId: id,metroName, metroNumber: trainNum, origin, destination, departureTime: startTime, arrivalTime, basePrice, totalSeats};

    await axios.put('http://localhost:6001/update-metro', inputs).then(
      async (response)=>{
    ...
    <div className='NewTrainPage'>
      ...
      <span className='newTrainSpan1'>
        <div className="form-floating mb-3">
                <input type="text" className="form-control" id="floatingInputemail" value={metroName} onChange={(e)=> setTrainName(e.target.value)} />
                <label htmlFor="floatingInputemail">metro Name</label>
          </div>
          <div className="form-floating mb-3">
                <input type="text" className="form-control" id="floatingInputmobile" value={trainNum} onChange={(e)=> setTrainNum(e.target.value)} />
                <label htmlFor="floatingInputmobile">metro Number</label>
          </div>
      </span>
      <span>
