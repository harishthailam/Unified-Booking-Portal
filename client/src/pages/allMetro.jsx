...
const AllTrains = () => {
  ...
  const fetchTrains = async () =>{
    await axios.get('http://localhost:6001/fetch-metro').then(
      (response)=>{
        ...
        {trains.map((train)=>{
          return(
              ...
              <div className="allTrains-train" key={train._id}>
                ...
                <span>
                  <p><b>Metro no:</b> {train.metroName}</p>
                  <p><b>Metro name:</b> {train.metroNumber}</p>
                </span>
                <span>
                ...
                <div>
                  <button className="btn btn-primary" onClick={()=> navigate(`/edit-metro/${train._id}`)}>Edit details</button>
                </div>
              </div>
          )
        })}
