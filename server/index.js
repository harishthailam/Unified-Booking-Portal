...
import { User, Train, Booking, Metro } from './schemas.js';
...
mongoose.connect('mongodb://127.0.0.1:27017/train', { 
    ...
    app.post('/add-metro', async (req, res)=>{
        const {metroName, metroNumber, origin, destination, departureTime, arrivalTime, basePrice, totalSeats} = req.body;
        try{

            const train = new Metro({metroName, metroNumber, origin, destination, departureTime, arrivalTime, basePrice, totalSeats});
            const newmetro = train.save();
    ...
    app.put('/update-metro', async (req, res)=>{
        const {metroId, metroName, metroNumber, origin, destination, departureTime, arrivalTime, basePrice, totalSeats} = req.body;
        try{

            const train = await Metro.findById(metroId)

            train.trainName = metroName;
            train.trainNumber = metroNumber;
            train.origin = origin;
            train.destination = destination;
            train.departureTime = departureTime;
            train.arrivalTime = arrivalTime;
    ...
    app.get('/fetch-metro', async (req, res)=>{
        
        try{
            const trains = await Metro.find();
            res.json(trains);
    ...
    app.get('/fetch-metro/:id', async (req, res)=>{
        const id = await req.params.id;
        console.log(req.params.id)
        try{
            const train = await Metro.findById(req.params.id);
            console.log(train);
            res.json(train);
